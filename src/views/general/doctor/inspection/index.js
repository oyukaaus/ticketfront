...
import {
    doctorInspectionIndex,
    appointmentRequestSubmit,
    appointmentRequestEdit,
    doctorInspectionSubmit
} from 'utils/fetchRequest/Urls';
import DeleteModal from 'modules/DeleteModal';
import ResponseForNewAppointment from './reponseForNewRequest';
import ResponseAppointment from './reponse';
import ViewAppointment from './view';
import RegisterInspection from './add';
...
const index = () => {
    ...
    const [showResponseForNewAppointment, setShowResponseForNewAppointment] = useState(false);
    ...
    const [showRegisterInspection, setShowRegisterInspection] = useState(false);

    const typeCode = 'DOCTOR';
    ...
    const breadcrumbs = [
        { to: '', text: t('menu.home') },
        { to: '/doctor/inspection', text: t('doctorsCorner.inspection') },

    ];

    const onResponseForNewAppointment = (newRequest) => {
        setSelectedRequest(newRequest)
        setShowResponseForNewAppointment(true);
    };
        ...
        fetchRequest(appointmentRequestEdit, 'POST', postData)
            .then(res => {
                ...
                if (success) {
                    ...
                    setAcceptedRequests(acceptedRequestsObj)
                } else {
        ...
        const tabs = {
                                ...
                                <Card.Body>
                                    <Row>
                                        ...
                                        <Col xs={3} className='d-flex align-items-center'>
                                            <Button
                                                onClick={() => onResponseForNewAppointment(newRequest)}
                                                variant="secondary"
                                                size='sm'
            ...
            'ACCEPT':
                acceptedRequests && acceptedRequests.length > 0 ?
                    acceptedRequests.map((acceptedRequest, i) => {
                        return (
                            <Card className={i > 0 ? 'no-border-radius mt-2' : 'no-border-radius'} key={i}>
                                <Card.Body>
                                    <Row>
                                        <Col xs={2} className='d-flex align-items-center'>
                                            <img className="profile" alt={acceptedRequest?.firstName || t('system.profilePicture')} src={acceptedRequest?.avatar ? `https://api.eschool.mn/${acceptedRequest?.avatar}` : '../img/system/default-profile.png'} width={65} />
                                        </Col>
                                        <Col xs={7}>
                                            <Row>
                                                <Col className='pe-0'>
                                                    <Button
                                                        ...
                                                        className='br-4 w-100 fs-13'
                                                    >
                                                        {acceptedRequest?.appointmentDate?.date?.slice(0, 10)}
                                                    </Button>
                                                </Col>
                                                <Col>
                                                    <Button
                                                        ...
                                                        className='br-4 w-100 fs-13'
                                                    >
                                                        {acceptedRequest?.startTime?.date?.slice(11, 16)} - {acceptedRequest?.endTime?.date?.slice(11, 16)}
                                                    </Button>
                                                </Col>
                                            </Row>
                                            <Row className='fs-13 mt-2'>
                                                <span className='color-grey'>{acceptedRequest?.createdDate?.date?.slice(0, 19)}</span>
                                                <span className='absolute-black'>{acceptedRequest?.className || '-'} | {acceptedRequest?.code || '-'}</span>
                                                <span className='absolute-black'>{acceptedRequest?.lastName} <b className='text-secondary text-uppercase font-weight-bold'>{acceptedRequest?.firstName}</b></span>
                                            </Row>
                                        </Col>
                                        <Col xs={3} className='d-flex align-items-center'>
                                            <Row>
                                                {
                                                    acceptedRequest?.visit && acceptedRequest?.visit?.length > 0 &&
                                                    <Col xs={12}>
                                                        <Button
                                                            // onClick={() => onResponseAppointment(acceptedRequest)}
                                                            style={{pointerEvents: 'none'}}
                                                            variant="success"
                                                            size='sm'
                                                            className='br-8 w-100 fs-11 text-white text-wrap lh-base p-1'
                                                        >
                                                            {t('doctorsCorner.answered')}
                                                        </Button>
                                                    </Col>
                                                }
                    ...
                    <Card className='no-border-radius'>
                        <Card.Body className='text-center'>
                            {t('appointment.emptyAcceptedRequest')}
                        </Card.Body>
                    </Card>,
        };
        return tabs[tabKey];
    ...
    useEffect(() => {
        ...
        fetchRequest(doctorInspectionIndex, 'POST', postData)
            .then(res => {
                const { success = false, message = null, appointments = [] } = res
                if (success) {
                    const scheduleObj = [];
    ...
    const onResponseForNewAppointmentSubmitHandler = (params) => {
        dispatch(setLoading(true));
        const postData = {
            school: selectedSchool?.id,
            ...params,
        ...
        fetchRequest(doctorInspectionSubmit, 'POST', postData)
            .then(res => {
                ...
                if (success) {
                    ...
                    setNewRequests(newRequestsObj)
                    setAcceptedRequests(acceptedRequestsObj)
                    setShowResponseForNewAppointment(false)
                    setShowRegisterInspection(false)
            ...
            <Row>
                ...
                <Col lg={12} className="mb-3">
                    <Button
                        ...
                        onClick={() => setShowRegisterInspection(true)}
                    >
                        <span className='m-0 font-weight-bold d-flex align-items-center br-8'>
                ...
                <Row>
                                ...
                                >
                                    {
                                        types.map(type => {
                                            return (
                                                <Col xxl={4} key={"type_" + type.id} className='p-0'>
                    ...
                    <Col md={8}>
                        ...
                        {/* Datatable end */}
                        {/* Modals start */}
                        {showResponseForNewAppointment &&
                            <ResponseForNewAppointment
                                newRequest={selectedRequest}
                                show={showResponseForNewAppointment}
                                setShow={setShowResponseForNewAppointment}
                                onSubmit={onResponseForNewAppointmentSubmitHandler}
                            />
                        }
                        ...
                        {showRegisterInspection &&
                            <RegisterInspection
                                show={showRegisterInspection}
                                setShow={setShowRegisterInspection}
                                onSubmit={onResponseForNewAppointmentSubmitHandler}
                            />
                        }
                        {/* Modals end */}
